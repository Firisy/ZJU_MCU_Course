...
uint8_t rotate_v=5;
uint16_t datac_pre[240]={0};
uint16_t datab_pre[240]={0};
    ...
    if (dataReady)
    {
      ...
      switch (command)
      {
        case IRR_Start:
          ...
          break;
        case IRR_Right:
          if(WorkState&NEC_Rotate_Right)
            rotate_v=(rotate_v)%5+1;
          else
          {
            if(WorkState&NEC_Rotate_Left) WorkState&=~NEC_Rotate_Left;
            WorkState |= NEC_Rotate_Right;
            rotate_v=1;
          }
          break;
        case IRR_Left:
          ...
          {
            if(WorkState&NEC_Rotate_Right) WorkState&=~NEC_Rotate_Right;
            WorkState |= NEC_Rotate_Left;
            rotate_v=1;
          }
          break;
        /*case IRR_Up:
          if(WorkState&NEC_Time_Adjust)
            date[0]=(date[0]+1)%10;
          else
          {
            mpoint += 5;
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /**/
  LL_GPIO_SetOutputPin(LED00_GPIO_Port, LED00_Pin);
  ...
  /**/
  LL_GPIO_SetPinPull(INT0_YK_GPIO_Port, INT0_YK_Pin, LL_GPIO_PULL_NO);

  /**/
  LL_GPIO_SetPinMode(INT0_IR_GPIO_Port, INT0_IR_Pin, LL_GPIO_MODE_INPUT);

  /**/
  LL_GPIO_SetPinMode(INT0_YK_GPIO_Port, INT0_YK_Pin, LL_GPIO_MODE_INPUT);
...
{
  DS1302Write(0x8e,0x00);//写入保护�???????????�???????????
  DS1302Write(0x80,0x20);//写入�???????????
  DS1302Write(0x82,0x27);//写入分钟
  DS1302Write(0x84,0x89);//写入小时
  DS1302Write(0x86,0x05);//写入�???????????
  DS1302Write(0x88,0x06);//写入�???????????
  DS1302Write(0x8a,0x03);//写入星期
  DS1302Write(0x8c,0x24);//写入�???????????
  DS1302Write(0x90,0x01);//充电
  DS1302Write(0xc0,0xf0);//初始化一次标�???????????
...
void Initial_c(void)
{
  ...
  0x7F,0x10,0x41,0x10,0x41,0x14,0x7F,0x32,0xFF,0x13,0x41,0xFE,0x03,0xFC,0x01,0x00}, //"�???????????"
  {0x00,0x00,0x02,0x00,0x2B,0xFF,0x2A,0xFE,0xFE,0xA8,0x7E,0xAA,0x2A,0xFF,0x2F,0xFE,
  0x1D,0x44,0xF5,0x42,0x67,0xFF,0x2F,0xFE,0x3D,0x40,0x77,0xE0,0x2F,0xC0,0x05,0x00}, //"�???????????"
  {0x00,0x00,0x07,0x00,0x18,0x00,0xFF,0xFF,0xFF,0xFE,0x0C,0x00,0x06,0x00,0x04,0x00,
  0x0E,0xFF,0x1C,0xFF,0x74,0x42,0xE4,0x42,0x44,0x42,0x1C,0x7F,0x0E,0xFE,0x04,0x40}, //"�???????????"
  };
  uint8_t i=0;
  uint8_t j=0;
  int k=16;
